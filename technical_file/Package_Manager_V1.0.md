...
而编程语言的包管理器可以想象成你在写作业时要用圆规、橡皮、尺子这些、这些工具是专用的，它是针对于这个事物专用的工具。

### 下载命令

这是一个宽泛的概念，包管理包含了下载这一功能，但我们使用一些工具也可以来下载我们想要的文件，比如说crul和wget，
...
它是基于dpkg工具做的包管理工具，dpkg工具只能处理.deb包文件的解包、安装等工作。

APT在此基础上添加了自动解决依赖安装、网络请求、软件更新等功能。
...
|  apt purge 包名  | 卸载软件包并删除所有配置文件 |
|  apt upgrade   |  升级所有已安装的软件包   |

如果是从网页上下载文件APT是做不了的，它只支持下载符合其包管理设计规范的特定文件，以及HTTP/HTTPS直链的.deb的软件。

#### Pacman

仅在Arch Linux系统下使用的包管理工具。
