...
这个包的功能可以理解为是Ubuntu服务器环境中主流的一个Web服务工具。
...
当我们下载的不是一个软件而是一个工具呢，比如说PyTorch。
...
而在 `Just download` 小节，虽然开头命令不同，一个用的是 `wget` ，另一个是 `curl` ，但都是在做相同的事情，从GitHub上下载一个可执行的脚本文件，并给这个文件可执行的权限。

`wget` 和 `curl` 都是下载工具。
 
那么包管理器是什么，网络请求工具又是什么呢。

包管理器就像是手机上的应用商店一样，当我们需要往电脑上安装软件的时候就可以从它那里去下载我们需要的应用。

包管理器的作用不光是帮你下载软件，它是你的软件管家，它能够帮你找合适的软件版本、下载软件安装包、安装软件、更新和卸载，这些你都不用操心，它自己就能帮你完成。

而包管理器也可以分为**系统级的包管理器**和**编程语言的包管理器**。

而下载工具就是一个比较宽泛的概念了，包管理器也包含了下载这一功能，但不同的下载命令还是有一些不同的。
...
下载一个软件包，一键安装、自动处理依赖、适配系统架构、申请管理员权限、无需手动点击。
...
在当前激活的环境中，自动解析并安装对应包的所有依赖，完成包的安装。
...
Node.js 官方默认的 JavaScript、TypeScript 编程语言级包管理器，负责前端框架、Node.js 工具库的安装与项目依赖管理。
