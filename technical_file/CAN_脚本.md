...
第一个是起始位，默认为0，然后接下去是仲裁段，包含11位的ID和RTR位，刚才发送的是0x230，这边解析出来也是0x230，RTR位为判断位，用来判断这一帧数据是数据帧还是遥控帧，之后是控制段，包含IDE位、r0位和DLC位，IDE位为判断位，用来判断这一帧数据是标准帧还是扩展帧，r0位为保留位，不做任何用处，为未来扩展提供可能性，DLC位为数据长度码，通过这一位可以控制发送数据段的长度
