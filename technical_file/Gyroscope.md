...
1. **第一步（转X轴）**：外环旋转，带动中环和内环一起动。
2. **第二步（转Y轴）**：关键点来了！当中间的Y轴正好**旋转了 ±90度** 时。
3. **结果**：这个90度的旋转，直接把最内层的**Z轴拉到了与最外层的X轴完全平行的位置**（两个轴重合了）。
4. **第三步（转Z轴）**：此时再旋转Z轴，其产生的运动效果，与第一步旋转X轴**完全在同一个平面上**。
